...
- [ ] **Step 4: Acceptance against the local Bamboo** (spec §10; FORGE-SPECS only; get an older SHA from `bam --server home build list FORGE-SPECS --json` revisions)

```bash
make install && bam version        # must show this branch's commit
bam --server home run FORGE-SPECS --revision <older full SHA> --watch
bam --server home build show --last          # Revisions shows that SHA
bam --server home run FORGE-SPECS --revision <older 7-char SHA> --watch
bam --server home run FORGE-SPECS --revision 0000000 --watch; echo "exit=$?"   # not built, message, exit=1
bam --server home run FORGE-SPECS --revision <SHA> --dry-run --json            # "revision", nothing triggered
make e2e ARGS='-server home -plan FORGE-SPECS -revision <older SHA>'
```

TUI in tmux: `bam --server home`, move to FORGE-SPECS, `R`, `j` to the revision row, `enter`, type the older SHA, `enter`, `ctrl-R`; the build's detail shows the SHA.
